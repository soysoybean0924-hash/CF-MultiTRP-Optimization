...
output.FinalPopulation=X; output.FinalObjectives=scores; output.FinalScores=[];
end
...
t.bestObjective=-inf; t.bestX=nan(1,D); t.bestResult=[];
t.traceObjective=nan(maxEval,1); t.traceScore=nan(maxEval,1);
t.traceBestObjective=nan(maxEval,1); t.traceBestScore=nan(maxEval,1);
...
try
    ...
    result=[]; objective=-realmax; score=-realmax;
end
t.evaluationCount=t.evaluationCount+1; k=t.evaluationCount;
t.traceObjective(k)=objective; t.traceScore(k)=score; t.archiveX(k,:)=x;
if isBetterCandidate(objective,score,t.bestObjective,t.bestScore)
    t.bestObjective=objective; t.bestScore=score; t.bestX=x; t.bestResult=result;
end
t.traceBestObjective(k)=t.bestObjective; t.traceBestScore(k)=t.bestScore;
if options.verbose

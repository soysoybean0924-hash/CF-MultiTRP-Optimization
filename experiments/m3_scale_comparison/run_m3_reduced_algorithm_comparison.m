...
fprintf(fid,'\n');
[~,bestIdx] = max(summaryTable.BestObjective);
fprintf(fid,'Best by objective: %s, Objective=%.6g, Score=%.6g, J_true=%.6g, SumRate=%.6g, Runtime=%.3fs\n\n', ...
    summaryTable.Method{bestIdx},summaryTable.BestObjective(bestIdx), ...
    summaryTable.BestScore(bestIdx),summaryTable.J_true(bestIdx), ...
    summaryTable.SumRate(bestIdx),summaryTable.RuntimeSeconds(bestIdx));
for i = 1:height(summaryTable)
    fprintf(fid,'- %-6s Objective=% .6g Score=% .6g J_true=% .6g SumRate=% .6g Jain=%.4f ActiveLinks=%d Runtime=%.3fs\n', ...
        summaryTable.Method{i},summaryTable.BestObjective(i),summaryTable.BestScore(i), ...
        summaryTable.J_true(i),summaryTable.SumRate(i),summaryTable.Jain(i),summaryTable.ActiveLinks(i), ...

...
rootFolder = fileparts(fileparts(resultsDir));
files = {'run_local_sensitivity_9params.m','cf_local_sensitivity_config.m', ...
    'cf_evaluate_three_objectives.m','cf_compute_true_objective.m', ...
    'cf_perturb_parameter.m','cf_plot_local_sensitivity.m', ...
    'cf_export_sensitivity_results.m'};
outFile = fullfile(resultsDir,'local_sensitivity_code_listing.txt');
fid = fopen(outFile,'w');
cleanup = onCleanup(@() fclose(fid));
for k = 1:numel(files)
    path = fullfile(rootFolder,files{k});
    fprintf(fid,'\n%%%% ===== %s =====\n',files{k});
    if exist(path,'file')
        text = fileread(path);
        fprintf(fid,'%s\n',text);
